                ...
                <tbody>
                    <?php if (!empty($usuarios)): ?>
                        <?php foreach ($usuarios as $u): ?>
                            <tr>
                                <td>
                                    <strong><?= esc($u['nombre'] . ' ' . $u['apellidos']) ?></strong>
                                </td>
                                <td>
                                    <i class="fas fa-envelope text-muted me-1"></i> <?= esc($u['correo']) ?><br>
                                    <i class="fas fa-phone text-muted me-1"></i> <small><?= esc($u['telefono']) ?></small>
                                </td>
                                <td>
                                    <?php if ($u['rol'] == 'admin'): ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (modalRevocar) {
            modalRevocar.addEventListener('show.bs.modal', function(event) {
                document.getElementById('btnEjecutarRevocar').setAttribute('href', event.relatedTarget.getAttribute('data-url'));
            });
        }
        var modalReactivar = document.getElementById('modalReactivar');
        if (modalReactivar) {
            modalReactivar.addEventListener('show.bs.modal', function(event) {
                document.getElementById('btnEjecutarReactivar').setAttribute('href', event.relatedTarget.getAttribute('data-url'));
            });
        }
    });
</script>
